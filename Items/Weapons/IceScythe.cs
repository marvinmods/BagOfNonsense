...
namespace BagOfNonsense.Items.Weapons
{
    public class IceScythe : ModItem
    {
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                Vector2 perturbedSpeed = new Vector2(speedX, speedY).RotatedBy(MathHelper.Lerp(-rotation, rotation, i / (3 - 1))) * .2f; // Watch out for dividing by 0 if there is only 1 projectile.
